...
class ChessBoard {
  ...
  boolean canMove (ChessPiece piece, int x, int y) {
	 for (int i = 0; i < pieces.size; i++) {
		 ChessPiece piece2 = pieces.get(i);
		 //check if there is a piece at square
		 if (piece2.col == x && piece2.row == y) {
			 //check to make sure pieces are different colors
			 if (piece.color == piece2.color) {
				 return false;
			 }
			 else {
				 //check if attack path is clear
